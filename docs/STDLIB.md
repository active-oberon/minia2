...
nanoseconds on Unix and `QueryPerformanceCounter` ticks on Windows, so every caller carried its
own division; and `Dates.Now` is the wall clock in local time with nothing saying which local.
`Time` gives the monotonic clock one unit everywhere and the wall clock an origin — 1970-01-01
UTC — plus the host offset out of `Clock.tz`. There is still no IANA zone database: naming a
zone, or asking what a past date's offset was, needs one.

**Formatting** (`source/Format.Mod`, `tests/Format.Test`). Every other language spells this
`printf`, and `printf` needs varargs, which Active Oberon does not have and should not grow for
this. What it has is objects, so the template is an object and the arguments arrive as calls on
it: `f := Format.To(w, "{} of {} failed"); f.Int(a, 0); f.Int(b, 0); f.Done`. The shape of the
message stays in one string — greppable, translatable — and every argument keeps the type the
compiler checked. A hole nobody filled is written out as `{}` rather than swallowed.

**Map and Set** (`source/GenericMaps.Mod` and `source/Hashing.Mod`, `tests/Maps.Test`). The
package had every container that is a line of elements and none that is a lookup, so anything
needing one grew its own — `TFClasses` has a string dictionary, `FoxBasic` a symbol table, and
neither can hand its map to the other, which is exactly what makes a map a std type. Same
parametric form as `GenericCollections`, from the same author's design: it compiles to a symbol
file and produces an object only when instantiated. `Hashing` is the pair of procedures a map
must be given; its first version used the rotate-and-xor that `FoxBasic` and `TFClasses` use and
put a thousand consecutive keys into two buckets — exclusive-or and rotation are linear, and the
multiplication of FNV-1a is what breaks the structure. The suite measures the spread.

**The UCD tables landed 2026-09-03** (`source/Unicode.Mod`, generated by `tests/gen-unicode.py`,
`tests/Unicode.Test`). The database was never missing from the tree — `UnicodeProperties` parses
`data/UnicodeData.txt` — but it parses it through `Texts` and `Codecs`, which reach the window
system, so a headless SDK could not have it and bare metal never will. The generator turns the
same file into a compiled run table: over the whole code space the triple (category, upper delta,
lower delta) is constant across **2642 runs** taking only **151 distinct values**, so a lookup is
a binary search and the module is 420 lines with no data file to ship. `UTF8Strings.ToUpper` and
`ToLower` are the first callers — the old `UpperCase` stays ASCII because `FATFiles` calls it with
one array as both arguments, and a simple case mapping can need a byte more than the character it
replaces. The file in the tree is **Unicode 5.0**: a newer table is a newer file in `data/` and
one script run.
...
`DynamicStrings`, `Format`, `NbrStrings`, `RealConversions`, `Regex`, `StringPool`, `Strings`, `TFStringPool`, `Texts`, `UTF8Strings`, `Unicode`, `UnicodeBidirectionality`, `UnicodeProperties`

`Regex` and `Unicode` both arrived 2026-09-03; nothing on this list is missing any more.

## std/time — 6

`ActiveTimers`, `Dates`, `PrecisionTimer`, `Stopwatch`, `Time`, `UpTime`

Missing, worth adding: **the tz database**. `Time` arrived 2026-09-03 with the monotonic clock
and UTC; naming a zone still needs IANA's tables. `Clock` stays in std/runtime — the kernel
needs it before any library does.

## std/data — 4

`GenericCollections`, `GenericMaps`, `GenericSort`, `Hashing`

`Map` and `Set` arrived 2026-09-03 in `GenericMaps`, parametric the way `GenericCollections`
is, with `Hashing` for the hash and equality a map has to be handed.
